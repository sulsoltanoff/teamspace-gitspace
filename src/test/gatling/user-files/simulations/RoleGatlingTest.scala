...
class RoleGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Role entity")
        ...
        .repeat(2) {
            ...
            .get("/services/gitspace/api/roles")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new role")
            .post("/services/gitspace/api/roles")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "name":"SAMPLE_TEXT"
                , "managePullRequests":null
                }""")).asJson
            .check(status.is(201))
            ...
            .repeat(5) {
                ...
                .get("/services/gitspace${new_role_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created role")
            .delete("/services/gitspace${new_role_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
